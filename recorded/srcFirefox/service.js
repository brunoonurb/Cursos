...
console.log('SERVICE');
class Service {
  processFile({ query, file, onOcurrenceUpdate, onProcess }) {
    const linesLength = { counter: 0 };
    const progressFn = this.#setupProgress(file.size, onProcess);
    ...
    file
      ...
      .pipeTo(this.#findOcurrences({ query, onOcurrenceUpdate: onUpdate() }));
    // .pipeTo(
    //   new WritableStream({
    //     write(chunk) {
  ...
  #setupProgress(totalBytes, onProcess) {
    let totalUploaded = 0;
    onProcess(0);
    return (chunkLength) => {
